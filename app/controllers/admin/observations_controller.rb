class Admin::ObservationsController < Admin::ApplicationController
  ...
  def create
    @observation = current_user.observations.build(allowed_params)
    if @observation.save
  ...
  def update
    @observation = Observation.find(params[:id])
    if @observation.update_attributes(allowed_params)
  ...
  private 
    ...
    def allowed_params
      params.require(:observation).permit([:citation_id, :event_id, :species_id, :method_id, :user_id, :value, :is_active])
    end
